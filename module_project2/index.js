...
const dbconfigSlave = require('./config/database-slave.js');
const connection = mysql.createConnection(dbconfig);
const readConnection = mysql.createConnection(dbconfigSlave);
const app = express();
...
app.get('/getId', (req, res) => {
  ...
  readConnection.query('select lpad(max(convert(substr('+attribute+','+(length+1)+','+(length+4)+'),signed)+1),4,0) as id from '+table,(error,rows) => {
    if (error) throw error;
    console.log('result is : ', rows);
    res.send(rows);
  });
...
app.get('/customers2/:id', (req, res) => {
  connection_slave.query('SELECT * from customer WHERE customer_id=\'' + req.params.id + '\'', (error, rows) => {
    if (error) throw error;
...
app.get('/customers', (req, res) => {
  readConnection.query('select * from customer', (error, rows) => {
    if (error) throw error;
...
app.get('/customer', (req, res) => {
  let id=req.param('id')
  readConnection.query('select * from customer WHERE customer_id=\''+id+'\'', (error, rows) => {
    if (error) throw error;
    console.log('Customer info is: ', rows);
    res.send(rows);
  });
...
app.get('/car', (req, res) => {
  let id=req.param('id')
  readConnection.query('select * from car_menu WHERE car_id=\''+id+'\'', (error, rows) => {
    if (error) throw error;
...
app.get('/cars', (req, res) => {
  readConnection.query('SELECT * FROM car_menu WHERE car_id NOT IN(SELECT car_menu_car_id FROM order_details)', (error, rows) => {
    if (error) throw error;
    console.log('Car info is: ', rows);
    res.send(rows);
  });
...
app.get('/salesperson', (req, res) => {
  let id=req.param('id')
  readConnection.query('select * from salesperson where salesperson_id=\''+id+'\'', (error, rows) => {
    if (error) throw error;
...
app.get('/carprice', (req, res) => {
  ...
  readConnection.query('select car_price from car_menu where car_id=\''+id+'\'', (error, rows) => {
    if (error) throw error;
...
app.get('/lookupInvoice', (req, res) => {
  let invoiceid=req.param('invoiceid');
  readConnection.query('SELECT ct.customer_name, cm.car_model, cm.car_color, cm.new_old_state, cm.serial_number, cm.car_year, cm.car_price, sp.salesperson_name FROM customer ct, car_menu cm, sales_invoice si, order_details od, salesperson sp WHERE si.customer_customer_id=ct.customer_id AND si.order_details_order_id=od.order_id AND od.car_menu_car_id=cm.car_id AND sp.salesperson_id=si.salesperson_salesperson_id AND si.invoice_id=\''+invoiceid+'\'',(error,rows) => {
    if (error) throw error;
    res.send(rows);
  });
});
...
app.get('/serviceChoice/yes', (req, res) => {
  let id=req.param('id');
  readConnection.query('SELECT DISTINCT product_name FROM product WHERE service_menu_service_id=\'' + id + '\'', (error, rows) => {
    if (error) throw error;
    res.send(rows);
  });
});
...
app.get('/lookupMechanic', (req, res) => {
  ...
  readConnection.query('SELECT mechanic_name FROM management m, mechanic mc WHERE m.mechanic_mechanic_id=mc.mechanic_id AND m.service_record_id=\''+serviceid+'\'',(error,rows) => {
    if (error) throw error;
    res.send(rows);
  });
});
...
app.get('/service', (req, res) => {
  readConnection.query('select * from service_menu', (error, rows) => {
    if (error) throw error;
    res.send(rows);
  });
});
...
app.get('/lookupMechanics', (req, res) => {
  readConnection.query('select * from mechanic', (error, rows) => {
    if (error) throw error;
    res.send(rows);
  });
});
...
app.get('/lookupTicket', (req, res) => {
  let custid=req.param('custid');
  readConnection.query('select service_ticket_id from service_records where customer_customer_id=\''+custid+'\'',(error,rows) => {
    if (error) throw error;
    res.send(rows);
  });
});
...
app.get('/serviceChoice', (req, res) => {
  let id=req.param('id');
  readConnection.query('select need_product from service_menu where service_id=\'' + id + '\'', (error, rows) => {
    if (error) throw error;
    res.send(rows);
  });
});
...
app.get('/checkTicket', (req, res) => {
  ...
  readConnection.query('select service_ticket_id from service_records where service_ticket_id=\''+ticket+'\'',(error,rows) => {
    if (error) throw error;
    res.send(rows);
  });
});
...
app.get('/service/getMecanic', (req, res) => {
  let serviceid=req.param('serviceid');
  readConnection.query('SELECT mc.mechanic_name from management ma, mechanic mc where ma.mechanic_mechanic_id=mc.mechanic_id AND service_record_id=\''+serviceid+'\'',(error,rows) => {
    if (error) throw error;
    res.send(rows);
  });
});
...
app.get('/service/getName', (req, res) => {
  let serviceid=req.param('serviceid');
  readConnection.query('SELECT sm.service_name FROM service_menu sm, service_records sr WHERE sm.service_id=sr.service_id AND sr.service_record_id=\''+serviceid+'\'',(error,rows) => {
    if (error) throw error;
    res.send(rows);
  });
});
...
app.get('/service/getCustName', (req, res) => {
  let serviceid=req.param('serviceid');
  readConnection.query('SELECT c.customer_name FROM customer c, service_records sr WHERE c.customer_id=sr.customer_customer_id AND sr.service_record_id=\''+serviceid+'\'',(error,rows) => {
    if (error) throw error;
    res.send(rows);
  });
});
...
app.get('/parts', (req, res) => {
  ...
  readConnection.query('select * from product_menu where parts_name like\'' + parts_name + '\'', (error, rows) => {
    if (error) throw error;
    res.send(rows);
  });
});
...
app.get('/product', (req, res) => {
  let id=req.param('id');
  readConnection.query('SELECT parts_name FROM product p, product_menu pm WHERE p.parts_id=pm.parts_id and p.parts_id=\'' + id + '\'', (error, rows) => {
    if (error) throw error;
    res.send(rows);
  });
});
...
app.get('/mechanicChoice', (req, res) => {
  let id=req.param('id');  
  readConnection.query('select mechanic_name from mechanic where mechanic_id=\'' +id+ '\'', (error, rows) => {
    if (error) throw error;
    res.send(rows);
  });
});
